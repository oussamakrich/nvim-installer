...
if [[ -d $HOME/.config/nvim ]]; then
	git -C $HOME/.config/nvim remote -v | grep -q "oussamakrich/nvim-config"
	if [[ $? -eq 0 ]]; then
		echo "pulling from oussamakrich/nvim-config"
		...
		git clone -q https://github.com/oussamakrich/nvim-config.git $HOME/.config/nvim
	fi
else
	echo "cloning from oussamakrich/nvim-config"
	git clone -q https://github.com/oussamakrich/nvim-config.git $HOME/.config/nvim
